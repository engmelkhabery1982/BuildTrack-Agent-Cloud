...
- `npm test` — exit 0, 274 tests passed, 0 failed (including all existing suites and new W05 tests).
...
- Targeted W05 governance tests (`node --test tests/cash-forecast-governance.test.mjs`) — exit 0, 14 tests passed, 0 failed.
...
- `cargo test --manifest-path src-tauri/Cargo.toml` — Cargo executable unavailable in Arena; Rust unit tests added to `src-tauri/src/cash_forecast_workflow.rs` and reserved for Codex local verification.
...
Evidence: Rust workflow `cash_forecast_workflow.rs`, Migration 77, and Tauri commands registered in `src-tauri/src/lib.rs`. 14 targeted integration tests passed. Cargo execution is pending Codex local verification because `cargo` is unavailable in Arena.
