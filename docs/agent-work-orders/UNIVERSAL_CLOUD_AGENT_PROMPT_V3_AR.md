...
جميع W04→W90 مفتوحة للتنفيذ كمرشحات، لكن نفذ **ميزة واحدة فقط لكل أمر مستخدم**.
ابدأ من `QUEUE_CURSOR.next_feature` ما لم يقل المستخدم صراحةً «ابدأ/نفذ الميزة التالية»؛
عندها اقرأ آخر handoff ناجح واختر الرقم التالي مباشرة حتى لو لم يحدث Codex `ACTIVE.md`.
ميزة واحدة = commit واحد + Result/Evidence واحد. بعد نجاح بوابتها حدّث cursor بحد أقصى
40 سطرًا إلى الرقم التالي ثم **توقف وانتظر رسالة المستخدم**. لا تنتظر قبول Codex أو تعديل
ملفات السلطة لكي تصبح الميزة التالية قابلة للتنفيذ، لكن لا تبدأها تلقائيًا.
...
إذا تعذر الدفع احتفظ بالcommit ولا تبدأ ميزة أخرى. الرد المختصر بعد كل ميزة:
`Wxx | commit | tests pass/fail/pending | next ready on user command`. ثم توقف. عندما يرسل
المستخدم «ابدأ الميزة التالية»، اجلب main إن أمكن وابدأ الرقم التالي دون انتظار Codex.
